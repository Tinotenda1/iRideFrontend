...
export const DriverOfferCard: React.FC<Props> = ({ 
  ...
  useEffect(() => {
    ...
    if (remainingMs <= 0) {
      slideOut(() => onExpire(offer.driver.phone));
      return;
    }
    ...
    <Animated.View style={[styles.card, { transform: [{ translateX: slideAnim }] }]}>
      <View style={[styles.content, isLocked && styles.lockedContent]}>
        ...
        <View style={styles.leftCol}>
          <IRAvatar source={{ uri: offer.driver.profilePic }} name={offer.driver.name} size="md" />
        ...
        <View style={styles.rightCol}>
          <View style={styles.headerRow}>
            ...
            <View style={styles.priceContainer}>
              ...
              <Text style={styles.priceText}>
                ${typeof offer.offer === 'number' ? offer.offer.toFixed(2) : offer.offer}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.vehicleRow}>
            ...
            <View style={styles.plateBadge}>
              <Text style={styles.plateText}>
                {offer.driver.vehicle.licensePlate || 'NO-PLATE'}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.footerRow}>
             <Text style={styles.etaText}>
               <Ionicons name="time-outline" size={14} /> {offer.eta || 'Nearby'}
             </Text>
             ...
             {isAccepted ? (
               ...
               <View style={styles.actionButtons}>
                 <TouchableOpacity onPress={() => slideOut(() => onDecline(offer.driver.phone))}>
                   <Text style={styles.declineBtn}>Decline</Text>
                 </TouchableOpacity>
                 <TouchableOpacity style={styles.acceptBtn} onPress={() => onAccept(offer)}>
...
const styles = StyleSheet.create({
  card: { backgroundColor: '#fff', borderRadius: 16, marginBottom: 10, overflow: 'hidden', elevation: 3, shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 4, shadowOffset: { width: 0, height: 2 } },
  lockedContent: { opacity: 0.7 },
  content: { flexDirection: 'row', padding: 12 },
  
  // Left Column
  leftCol: { alignItems: 'center', width: 60 },
  ratingBadge: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F8FAFC', paddingHorizontal: 6, borderRadius: 10, marginTop: 4 },
  ratingText: { fontSize: 10, fontWeight: 'bold', marginLeft: 2 },
  ridesText: { fontSize: 10, color: '#94a3b8', marginTop: 3, fontWeight: '500' }, // Added style for rides

  // Right Column
  rightCol: { flex: 1, marginLeft: 12 },
  headerRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }, // Changed to flex-start for badge alignment
  driverName: { fontWeight: '700', fontSize: 16, color: '#1e293b', marginTop: 2 },
  
  // Price & Badges
  priceContainer: { alignItems: 'flex-end' },
  priceText: { fontWeight: '800', color: '#10B981', fontSize: 18 },
  yourFareBadge: { backgroundColor: '#DCFCE7', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4, marginBottom: 2 },
  yourFareText: { fontSize: 9, color: '#166534', fontWeight: '700', textTransform: 'uppercase' },
  ...
  plateBadge: { backgroundColor: '#f1f5f9', paddingHorizontal: 4, paddingVertical: 1, borderRadius: 4, borderWidth: 1, borderColor: '#e2e8f0' },
  plateText: { fontSize: 10, color: '#475569', fontWeight: '600', textTransform: 'uppercase' },
  ...
  statusBadgeProcessing: { backgroundColor: '#E0F2FE', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8 },
  waitingText: { color: '#0284c7', fontWeight: '800', fontSize: 11 },
  statusBadgeAccepted: { backgroundColor: '#10B981', flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8 },
